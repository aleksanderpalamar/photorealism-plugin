...
pub fn build(layout: &Layout, settings: &Settings, title: &str) -> Vec<Primitive> {
...
mod tests {
    ...
    fn the_panel_and_the_title_bar_come_first() {
        let layout = layout();
        let primitives = build(&layout, &Settings::default(), "menu");
    ...
    fn nothing_is_drawn_outside_the_panel() {
        let layout = layout();
        let primitives = build(&layout, &Settings::default(), "photorealism-plugin");
    ...
    fn a_longer_title_produces_more_glyphs() {
        let layout = layout();
        let short = build(&layout, &Settings::default(), "a");
        let long = build(&layout, &Settings::default(), "abcd");
    ...
    fn characters_without_a_glyph_are_skipped() {
        let layout = layout();
        let plain = build(&layout, &Settings::default(), "abc");
        let accented = build(&layout, &Settings::default(), "abcá");
    ...
    fn a_locked_row_is_painted_with_the_locked_color() {
        let layout = layout();
        let automatic = build(&layout, &Settings::default(), "menu");
        let fixed = build(
            &layout,
            &Settings {
    ...
    fn turning_a_switch_off_removes_its_inner_mark() {
        let layout = layout();
        let on = build(&layout, &Settings::default(), "menu");
        let off = build(
            &layout,
            &Settings {
                enabled: false,
